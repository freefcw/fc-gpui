...
impl Bounds<Pixels> {
    ...
    ///
    /// Centers within the display's work area (`visible_bounds`) and clamps each axis with
    /// [`Size::min`] so the window cannot exceed that area. Callers that already fit are
    /// unchanged except for using the work-area center instead of the full display center.
